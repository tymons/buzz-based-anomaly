...
class ContrastiveConv1DVAE(cvbm.ContrastiveVariationalBaseModel):
    def __init__(self, model_type: HiveModelType, features: List[int], dropout_probs: List[float], kernel_size: int,
                 padding: int, max_pool: int, latent_size: int, input_size: int, tc_alpha: float = 0.1,
                 tc_loss: bool = False):
        ...
        self.decoder = Conv1DDecoder(features[::-1], dropout_probs[::-1], kernel_size, padding, latent_size,
